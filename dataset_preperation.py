...
class RoadSkeletonDataset(torch.utils.data.Dataset):
    ...
    def add_distortions(self,image, noise_prob=0.8, blur_prob=0.6):
        """
        ...
        if random.random() < noise_prob:
            ...
            img = self.add_noise(img * 255, noise_type) / 255.0
         # Potentially add blur
        if random.random() < blur_prob:
            blur_type = random.choice(['gaussian', 'motion', 'defocus'])
            img = self.add_blur(img * 255, blur_type) / 255.0
    ...
    def __getitem__(self, idx):
        ...
        #input_array = thick_img  # Convert from (1, H, W) to (H, W)
        #img = Image.fromarray(input_array, mode='L')  # 'L' for grayscale images
        #img.show()
        # Apply distortions to the INPUT image (thick road)
        if self.apply_distortions:
            thick_img = self.add_distortions(thick_img)
...
RS = RoadSkeletonDataset(thinning_dir = "/home/aljadaaa/Documents/skelnton/Road_Skeletonization/thinning_data/data/thinning", target_png_dir = "/home/aljadaaa/Documents/skelnton/Road_Skeletonization/thinning_data/data/targets_png", geojson_dir = "/home/aljadaaa/Documents/skelnton/Road_Skeletonization/thinning_data/data/targets_geojson")

"""
just for testing purposes
data_item = RS.__getitem__(199)
#print(RS.__getitem__(1))
"""
...
def visualize_dataset_samples(thinning_dir, target_png_dir, num_samples=3):
    ...
    for i in range(min(num_samples, len(indices))):
        ...
        # Apply distortions to the INPUT image (thick road)
        distorted_img = RS.add_distortions(thick_img)
...
def test_distortion_grid(image_path, num_rows=2, num_cols=3):
    ...
    distortion_types = [
        ('Gaussian Noise', lambda img: RS.add_noise(img, 'gaussian')),
        ('Salt & Pepper Noise', lambda img: RS.add_noise(img, 'salt_pepper')),
        ('Gaussian Blur', lambda img: RS.add_blur(img, 'gaussian')),
        ('Motion Blur', lambda img: RS.add_blur(img, 'motion')),
        ('Combined (Noise+Blur)', lambda img: RS.add_distortions(img))
    ]
