...
#bce_dice = DiceLoss(mode='bce')  

dice = DiceLoss(sigmoid=True, reduction='mean')
bce  = BCEWithLogitsLoss()

def bce_dice_combined(logits, targets, alpha=0.5):
    return alpha * bce(logits, targets) + (1 - alpha) * dice(logits, targets)
class Config:
    def __init__(self):
        self.batch_size = 8
        self.learning_rate = 1e-5#1e-4 
        self.epochs = 250
...
if __name__ == "__main__":
    ...
    # Train, i was using critreion, now let's try dice
    #train_losses, val_losses = train_model(train_loader, val_loader, model, bce_dice_combined, optimizer, config)
    """
    ...
    # Evaluate
    model.load_state_dict(torch.load("/home/aljadaaa/Documents/Road_Skeletonization/aibak_code/unet_skeleton.pth", map_location=config.device))
    model.eval()
